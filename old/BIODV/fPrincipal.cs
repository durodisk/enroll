...
namespace BIODV
{
	public class fPrincipal : Form
	{
		...
		private DataGridViewTextBoxColumn ArchivoEpd;
		...
		private RibbonOrbRecentItem ribbonOrbRecentItem4;
		...
		static fPrincipal()
		{
			fPrincipal.dt_enprogreso = null;
			fPrincipal.dt_pendientes = null;
			fPrincipal.dt_enviados = null;
			fPrincipal.link = "PRO";
		}
		...
		private void btnEnvio_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					BIODV.Util.Serializer ser = new BIODV.Util.Serializer();
					DataGridViewRow vFila = this.dgPendientes.Rows[this.dgPendientes.CurrentCell.RowIndex];
					fPrincipal.RutaEpd = vFila.Cells[7].Value.ToString();
					...
					if (MessageBox.Show(string.Concat(new string[] { "Se sincronizara el registro:\n\nNombre Completo: ", nombre, "\nIdentificacion: ", identificacion, "\n\n¿Esta seguro de continuar?" }), "Mensaje", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
					{
						...
						fPrincipal.PersonaCapturada = ser.DeserializeEpd(fPrincipal.RutaEpd);
						if (fPrincipal.PersonaCapturada.BasicData.UploadStatus.ToString() == "Pending")
						{
							form.Show();
							form.IniciarSincronizacionRegistro(fPrincipal.RutaEpd);
						}
						if (fPrincipal.PersonaCapturada.BasicData.UploadStatus.ToString() == "Done")
						{
							form.Show();
							form.IniciarSincronizacionUpdate(fPrincipal.RutaEpd);
						}
					}
				}
			}
		...
		private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
		{
			try
			{
				...
				if (this.rowIndex > -1)
				{
					...
					fPrincipal.RutaEpd = vFila.Cells["ArchivoEpdN"].Value.ToString();
					(new fEnrolar()
					{
		...
		private void fPrincipal_Load(object sender, EventArgs e)
		{
			...
			try
			{
				...
				flag = (fLogin.usuario == "admin.policia" ? true : fLogin.usuario == "estanislao.castaya");
				flag1 = flag;
		...
		{
			ComponentResourceManager resources = new ComponentResourceManager(typeof(fPrincipal));
			...
			base.Name = "fPrincipal";
			base.StartPosition = FormStartPosition.CenterScreen;
			base.WindowState = FormWindowState.Maximized;
			base.Load += new EventHandler(this.fPrincipal_Load);
		...
		private void LlenarGrilla()
		{
			...
			try
			{
				...
				fPrincipal.dt_enprogreso = vPersonas.Clone();
				fPrincipal.dt_pendientes = vPersonas.Clone();
				fPrincipal.dt_enviados = vPersonas.Clone();
				this.con_dgdat = 0;
				this.con_dgpen = 0;
				this.con_dgsub = 0;
				...
				for (int i = 0; i < (int)strArrays.Length; i++)
				{
					...
					CapturedPerson PersonaCapturada = (new BIODV.Util.Serializer()).DeserializeEpd(vArchivoEpd);
					...
					{
						this.con_dgdat++;
						if (fPrincipal.link == "PRO")
						{
							fPrincipal.dt_enprogreso.Rows.Add(new object[] { img, PersonaCapturada.BasicData.FullName, identificacion, PersonaCapturada.BasicData.Created, PersonaCapturada.BasicData.CreatedBy, PersonaCapturada.BasicData.ModifiedBy, GeneralCompleted, vArchivoEpd });
						}
					}
					...
					{
						this.con_dgpen++;
						if (fPrincipal.link == "PEN")
						{
							fPrincipal.dt_pendientes.Rows.Add(new object[] { img, PersonaCapturada.BasicData.FullName, identificacion, PersonaCapturada.BasicData.Created, PersonaCapturada.BasicData.CreatedBy, PersonaCapturada.BasicData.ModifiedBy, GeneralCompleted, vArchivoEpd });
						}
					}
					...
					{
						this.con_dgsub++;
						if (fPrincipal.link == "ENV")
						{
							fPrincipal.dt_enviados.Rows.Add(new object[] { img, PersonaCapturada.BasicData.FullName, identificacion, PersonaCapturada.BasicData.Created, PersonaCapturada.BasicData.CreatedBy, PersonaCapturada.BasicData.ModifiedBy, GeneralCompleted, vArchivoEpd, PersonaCapturada.BasicData.StatusMessage });
						}
					}
				}
				...
				{
					this.dgDatos.DataSource = fPrincipal.dt_enprogreso;
					this.dgDatos.RowTemplate.Height = 30;
					this.dgDatos.Visible = true;
				}
				if ((this.dgDatos.Visible || !this.dgPendientes.Visible ? false : !this.dgSubidos.Visible))
				{
					this.dgPendientes.DataSource = fPrincipal.dt_pendientes;
					this.dgPendientes.RowTemplate.Height = 30;
					this.dgPendientes.Visible = true;
				}
				if ((this.dgDatos.Visible || this.dgPendientes.Visible ? false : this.dgSubidos.Visible))
				{
					this.dgSubidos.DataSource = fPrincipal.dt_enviados;
		...
		private void ribbonButton5_Click(object sender, EventArgs e)
		{
			...
			if (this.dgDatos.Visible)
			{
				if (this.dgDatos.CurrentCell.RowIndex > -1)
				{
					...
					fPrincipal.RutaEpd = ((DataGridViewTextBoxCell)vFila.Cells[7]).Value.ToString();
					fPrincipal.RutaEpd.Split(new char[] { '\\' });
					if (File.Exists(fPrincipal.RutaEpd))
					{
						fPrincipal.RutaEpd = Path.GetFullPath(fPrincipal.RutaEpd);
						Process.Start("explorer.exe", string.Format("/select,\"{0}\"", fPrincipal.RutaEpd));
					}
				}
			}
				...
				try
				{
					if (this.dgPendientes.CurrentCell.RowIndex > -1)
					{
						...
						if (MessageBox.Show(string.Concat(new string[] { "Nombre : ", nombre, "\nIdentificacion: ", identificacion, "\n\n¿Esta seguro(a) de Autorizar la edicion del Registro?" }), "Mensaje", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							...
							if (fPrincipal.PersonaCapturada.BasicData.UploadStatus.ToString() == "Pending")
							{
								fPrincipal.PersonaCapturada.BasicData.UploadStatus = UploadStatus.InProgress;
							}
							if (fPrincipal.PersonaCapturada.BasicData.UploadStatus.ToString() == "Done")
							{
								fPrincipal.PersonaCapturada.BasicData.UploadStatus = UploadStatus.ManualVerification;
							}
							ser.SerializeEpd(fPrincipal.PersonaCapturada, fPrincipal.RutaEpd);
							this.LlenarGrilla();
						}
					}
				}
			...
			if ((this.dgPendientes.Visible ? false : this.dgSubidos.Visible))
			{
				try
				{
					if (this.dgSubidos.CurrentCell.RowIndex > -1)
					{
						...
						if (MessageBox.Show(string.Concat(new string[] { "Nombre : ", nombre, "\nIdentificacion: ", identificacion, "\n\n¿Esta seguro(a) de Autorizar la edicion del Registro?" }), "Mensaje", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
						{
							if (fPrincipal.PersonaCapturada.BasicData.UploadStatus.ToString() != "UploadingBiometricData")
							{
			...
			try
			{
				if (MessageBox.Show("¿Esta seguro de que desea eliminar los elementos seleccionados?", "Enrolamiento", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
				{
					if (this.dgDatos.CurrentCell.RowIndex > -1)
					{
						...
						if (File.Exists(fPrincipal.RutaEpd))
						{
							File.Delete(fPrincipal.RutaEpd);
						}
					}
					this.LlenarGrilla();
				}
		...
		private void ribtnNuevo_Click(object sender, EventArgs e)
		{
			try
			{
				fPrincipal.RutaEpd = string.Empty;
				(new fEnrolar()
				{
					MdiParent = base.ParentForm
				}).Show();
		...
		private void textBox1_TextChanged(object sender, EventArgs e)
		{
			try
			{
				...
				{
					this.dgDatos.Visible = true;
					DataRow[] dr = fPrincipal.dt_enprogreso.Select(string.Concat("Nombre LIKE '%", this.textBox1.Text, "%'"));
					if (dr.Length == 0)
					{
						DataRow[] dr0 = fPrincipal.dt_enprogreso.Select(string.Concat("Identificacion LIKE '%", this.textBox1.Text, "%'"));
						if (dr0.Length == 0)
						{
							DataRow[] dr1 = fPrincipal.dt_enprogreso.Select(string.Concat("Fecha LIKE '%", this.textBox1.Text, "%'"));
							if (dr1.Length == 0)
							{
								DataRow[] dr2 = fPrincipal.dt_enprogreso.Select(string.Concat("Creado LIKE '%", this.textBox1.Text, "%'"));
								if (dr2.Length == 0)
								{
									this.dgDatos.Visible = false;
								}
				...
				else if (fPrincipal.link != "PEN")
				{
					this.dgSubidos.Visible = true;
					DataRow[] dr = fPrincipal.dt_enviados.Select(string.Concat("Nombre LIKE '%", this.textBox1.Text, "%'"));
					if (dr.Length == 0)
					{
						DataRow[] dr0 = fPrincipal.dt_enviados.Select(string.Concat("Identificacion LIKE '%", this.textBox1.Text, "%'"));
						if (dr0.Length == 0)
						{
							DataRow[] dr1 = fPrincipal.dt_enviados.Select(string.Concat("Fecha LIKE '%", this.textBox1.Text, "%'"));
							if (dr1.Length == 0)
							{
								DataRow[] dr2 = fPrincipal.dt_enviados.Select(string.Concat("Creado LIKE '%", this.textBox1.Text, "%'"));
								if (dr2.Length == 0)
								{
									this.dgSubidos.Visible = false;
								}
				...
				{
					this.dgPendientes.Visible = true;
					DataRow[] dr = fPrincipal.dt_pendientes.Select(string.Concat("Nombre LIKE '%", this.textBox1.Text, "%'"));
					if (dr.Length == 0)
					{
						DataRow[] dr0 = fPrincipal.dt_pendientes.Select(string.Concat("Identificacion LIKE '%", this.textBox1.Text, "%'"));
						if (dr0.Length == 0)
						{
							DataRow[] dr1 = fPrincipal.dt_pendientes.Select(string.Concat("Fecha LIKE '%", this.textBox1.Text, "%'"));
							if (dr1.Length == 0)
							{
								DataRow[] dr2 = fPrincipal.dt_pendientes.Select(string.Concat("Creado LIKE '%", this.textBox1.Text, "%'"));
								if (dr2.Length == 0)
								{
									this.dgPendientes.Visible = false;
								}
